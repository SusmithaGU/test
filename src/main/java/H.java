...
public class H {
    ...
    public static String k() throws GeneralSecurityException {

        PrivateKey privatekey=getPrivateKeyFromString(key);
        String s = Jwts.builder()
                .setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + (1000 * 60 * 10))).setIssuer("176843")
                .signWith(SignatureAlgorithm.RS256,privatekey).compact();
        return s;
    }
    public static Response v() throws IOException, GeneralSecurityException {

        Headers.Builder headers = new Headers.Builder();
        headers.add("Accept: application/vnd.github.v3+json");
        headers.add("Authorization: Bearer "+k());
        Request request = new Request.Builder().post(h()).headers(headers.build()).url("https://api.github.com/app/installations/23753465/access_tokens").build();
        OkHttpClient o = new OkHttpClient();
        Response t = o.newCall(request).execute();
     return t;
    }
    public static RequestBody h() throws GeneralSecurityException {
        RequestBody r3= new FormBody.Builder().build();
        return r3;
    }
    @Data
    ...
    public static void main(String[] args) throws IOException, GeneralSecurityException {
       ...
       // ContentMetadata metadata= objectMapper.readValue(response.string(), ContentMetadata.class);
